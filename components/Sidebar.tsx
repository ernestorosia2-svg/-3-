...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const ChapterList = ({ chapters, label }: { chapters: Chapter[], label: string }) => {
    ...
    return (
      <div className="mb-6">
        <h3 className="px-4 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider sticky top-0 bg-slate-900 z-10">
          {label}
        </h3>
      ...
      <div className="p-6 border-b border-slate-700 bg-slate-900 flex-shrink-0">
        <div className="flex items-center space-x-3 text-white">
          ...
          <div>
            ...
            <p className="text-xs text-blue-400 font-medium">Review Companion</p>
          </div>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-slate-700">
        <ChapterList chapters={volume1} label="第一册 (Volume 1)" />
        <ChapterList chapters={volume2} label="第二册 (Volume 2)" />
      </div>
